...
func TestDatabase(t *testing.T) {
	...
	t.Run("GetBudgetStatus", func(t *testing.T) {
		...
		rows, err := conn.Query(
			ctx,
			"SELECT * FROM data.budget_status WHERE id IN (SELECT id FROM data.accounts WHERE ledger_id = $1)",
			ledgerID,
		)
		...
		rows, err = conn.Query(
			ctx,
			"SELECT * FROM data.budget_status WHERE id IN (SELECT id FROM data.accounts WHERE ledger_id = $1) AND account_name = 'Groceries'",
			ledgerID,
		)
		is.NoErr(err) // Should query budget status without error
		defer rows.Close()
