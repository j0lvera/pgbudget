...
func TestDatabase(t *testing.T) {
	...
	t.Run("GetBudgetStatus", func(t *testing.T) {
		...
		rows, err := conn.Query(
			ctx,
			"SELECT * FROM api.get_budget_status($1) AS budget_status(id, account_name, budgeted, activity, balance)",
			ledgerID,
		)
		...
		rows, err = conn.Query(
			ctx,
			"SELECT * FROM api.get_budget_status($1) AS budget_status(id, account_name, budgeted, activity, balance) WHERE account_name = 'Groceries'",
			ledgerID,
		)
		is.NoErr(err) // Should query budget status without error
		defer rows.Close()
